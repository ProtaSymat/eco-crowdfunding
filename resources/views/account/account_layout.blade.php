...
<div class="container py-5">
    ...
    <i data-feather="user" class="feather-sm me-2"></i>Mon profil
                    ...
                    <a href="{{ route('user.contributions') }}" class="list-group-item list-group-item-action {{ request()->routeIs('user.contributions') ? 'active' : '' }}">
                        <i data-feather="heart" class="feather-sm me-2"></i>Mes contributions
                    </a>
                    
                    @if(Auth::user()->role === 'backer')
                    <button onclick="document.getElementById('become-creator-form').submit();">Devenir Créateur</button>

                    <form id="become-creator-form" action="{{ route('become.creator') }}" method="POST" style="display: none;">
                        @csrf
                    </form>
                    @endif
                    ...
                    @if(Auth::user()->role === 'creator' || Auth::user()->role === 'admin')
                        ...
                        <a href="{{ route('creator.project.create') }}" class="list-group-item list-group-item-action {{ request()->routeIs('creator.project.create') ? 'active' : '' }}">
                            <i data-feather="plus-circle" class="feather-sm me-2"></i>Créer un projet
                        </a>
                        <a href="{{ route('creator.analytics') }}" class="list-group-item list-group-item-action {{ request()->routeIs('creator.analytics') ? 'active' : '' }}">
                            <i data-feather="bar-chart-2" class="feather-sm me-2"></i>Statistiques
                        </a>
                    @endif
                    ...
                    @if(Auth::user()->role === 'admin')
                        ...
                        <a href="{{ route('admin.contributions') }}" class="list-group-item list-group-item-action {{ request()->routeIs('admin.contributions') ? 'active' : '' }}">
                        ...
                        <a href="{{ route('admin.logs') }}" class="list-group-item list-group-item-action {{ request()->routeIs('admin.logs') ? 'active' : '' }}">
